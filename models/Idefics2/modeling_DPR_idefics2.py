...
class Idefics2Model(Idefics2PreTrainedModel):
    def __init__(self, config: Idefics2Config):
        ...
        # DPR Layer: low version에는 없음. 지워야함.
        self.dpr = DPRCrossAttention(n_split=9, image_seq_len=64, input_dim=4096)  
        self.mlp = Mlp()
    ...
    def forward(
        ...
        image_hidden_states = self.mlp(image_hidden_states)
...
class DPRCrossAttention(nn.Module):
    def __init__(self, n_split=9, image_seq_len=64, input_dim=4096):
        super().__init__()
    ...
    def forward(self, id, src, tgt): # x.shape (batch_size, seq_length, input_dim)
        ...
        score = torch.bmm(queries, keys.transpose(1, 2))/(self.input_dim**0.5)      # (1, text_token, 64*n_split)
...
class Mlp(nn.Module):
    def __init__(self, in_features=4096, hidden_features=4096, out_features=4096, act_layer=nn.GELU, drop=0.2):
        super().__init__()
