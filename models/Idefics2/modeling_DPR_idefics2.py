...
class Idefics2Model(Idefics2PreTrainedModel):
    ...
    def forward(
        ...
        # Regressive Predict Next Token
        if input_ids.shape[1] == 1:
            pass
            # image_token_len = image_hidden_states.shape[1]
            # image_token = torch.tensor([[32001]*image_token_len]).to(device=input_ids.device)
            # input_ids = torch.cat([input_ids, image_token], 1)
            # inputs_embeds = self.text_model.get_input_embeddings()(input_ids)
        else:
            # Dynamic Patch Retrieval ===========================================
            dpr_multihead_out = []
            for dpr_layer in self.DPR_module_list:
                dpr_head_out = dpr_layer(input_ids, inputs_embeds, image_hidden_states)
                dpr_multihead_out.append(dpr_head_out)
            dpr_multihead_out = torch.cat(dpr_multihead_out, dim=-1)
            image_hidden_states = self.mlp(dpr_multihead_out)
...
class DPRCrossAttention(nn.Module):
    def __init__(self, n_split=9, image_seq_len=64, input_dim=4096, heads=4):
        ...
        self.query = nn.Linear(self.input_dim, self.hidden_dim) # [batch_size, seq_length, input_dim]
        self.key = nn.Linear(self.input_dim, self.hidden_dim) # [batch_size, seq_length, input_dim]
    ...
    def forward(self, id, src, tgt): # x.shape (batch_size, seq_length, input_dim)
        ...
        tgt_idx = (id!=self.img_token)
        tgt = tgt.view(-1, self.n_split*self.image_seq_len, self.input_dim)         # (1, 64*n_split, 4096)
        
        src=src[tgt_idx]
        src=src.view(batch, -1, self.input_dim)
        
        queries = self.query(src)                                                   # (1, text_token, 4096)
        keys = self.key(tgt)                                                        # (1, 64*n_split, 4096)
        values = self.value(tgt)                                                    # (1, 64*n_split, 4096)

        score = torch.bmm(queries, keys.transpose(1, 2))/(self.hidden_dim**0.5)      # (1, text_token, 64*n_split)
        attention = self.softmax(score)                                             # (1, text_token, 64*n_split)
        attention = self.attn_drop(attention)
        
        weighted = torch.bmm(attention, values)                                     # (1, text_token, input_dim)
...
class Mlp(nn.Module):
    def __init__(self, input_dim=4096, act_layer=nn.GELU, drop=0.2):
        super().__init__()
        self.input_dim = input_dim
