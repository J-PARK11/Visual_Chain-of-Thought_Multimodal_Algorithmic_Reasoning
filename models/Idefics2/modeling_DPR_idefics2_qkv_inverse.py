...
class Idefics2Model(Idefics2PreTrainedModel):
    def __init__(self, config: Idefics2Config):
        ...
        n_split = 5
        print(f'Num of image split: {n_split}')
        self.DPR_module_list = nn.ModuleList()
        for i in range(self.heads):
            self.DPR_module_list.append(DPRCrossAttention(n_split=n_split, image_seq_len=64, input_dim=4096, heads=4, drop=0.3))
    ...
    def forward(
        ...
        if input_ids.shape[1] == 1:
            ...
            dpr_multihead_out += image_hidden_states
            dpr_multihead_out = dpr_multihead_out.view(batch,-1,4096)
            image_hidden_states = self.DPR_mlp(dpr_multihead_out)
            image_hidden_states = image_hidden_states.view(-1,64,4096)
...
class DPRCrossAttention(nn.Module):
    ...
    def forward(self, id, src, tgt): 
        ...
        tgt = tgt.view(-1, self.n_split*self.image_seq_len, self.input_dim)        
        
        src=src[tgt_idx]
        src=src.view(batch, -1, self.input_dim)
        
        queries = self.query(tgt)                                                 
        keys = self.key(src)                                                        
        values = self.value(src)                                                  

        score = torch.bmm(queries, keys.transpose(1, 2))/(self.hidden_dim**0.5)     
        ...
        weighted = weighted.view(batch*self.n_split, -1, self.hidden_dim)
