...
data_dir = '/data/SMART101/MathVerse'
img_dir = '/data/SMART101/MathVerse/images'
filename = '/data/SMART101/MathVerse/testmini.json'
df = pd.read_json(filename)

def generate_caption():
    base_directory = '/data/SMART101/MathVerse/captions/'
    json_files = glob.glob(os.path.join(base_directory, '*', '*.json'), recursive=True)
    caption_dict = dict()

    for json_file in json_files:
        cur_path = json_file.split('/data/SMART101/MathVerse/captions/')[1].split('/')[0]
...
def generate_mathverse_qainfo(filename):
    df = pd.read_json(filename)
    count = 0
    mc_count = 0
    no_ques = 0
...
# mathverse_qa_dict = generate_mathverse_qainfo(filename)
